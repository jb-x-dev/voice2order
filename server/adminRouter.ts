...
// Import Platzl order data from Excel
async function importPlatzlData(userId: string) {
  ...
  if (existing.length > 0) {
    return { message: "Data already imported", skipped: true };
  }
  ...
  try {
    ...
    throw new Error("Platzl data file not found. Please run the import script first.");
  }
...
export const adminRouter = router({
  seedData: protectedProcedure.mutation(async ({ ctx }) => {
    try {
      const result = await importPlatzlData(ctx.user.id);
      return result;
    } catch (error: any) {
